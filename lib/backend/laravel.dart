...
class apiResponse {
  ...
  static Future<SearchModel> apiSearch(
    ...
    try {
      if (message.isEmpty != true) {
        ...
        var Response = await http.post(Uri.parse(apiUrl),body: {'content': message},);
        if (Response.statusCode == 200) {
  ...
  static Future<void> apiNewPassword(
      {required BuildContext context,required String email,required String password,required String password_confirmation,required String otp}) async {
    try {
      String apiUrl = '${baseUrl}api/user/reset/by-otp';
      var Response = await http.post(Uri.parse(apiUrl),body: {'email': email,'password': password,'password_confirmation': password_confirmation,'otp': otp},);
      if (Response.statusCode == 200) {
        Map<String, dynamic> userData = jsonDecode(Response.body);
  ...
  Future<void> apiUserRegistration({required BuildContext context,required String name,required String email,required String password,required String password_confirmation}) async {
    String apiUrl = '${baseUrl}api/user/register';
    try {
      var response = await http.post(Uri.parse(apiUrl),body: {"name": name,"email": email,"password": password,"password_confirmation": password_confirmation,},);
      if (response.statusCode == 200) {
        Map<String, dynamic> userData = jsonDecode(response.body);
  ...
  Future<void> apiResetPassword({required BuildContext context,required String email,
  }) async {
    try {
      String apiUrl = '${baseUrl}api/user/password-reset';
      http.Response response = await http.post(Uri.parse(apiUrl),body: {'email': email},);
      if (response.statusCode == 200) {
        Map<String, dynamic> userData = jsonDecode(response.body);
        String successful = userData['message'];
  ...
  static Future<List<chapterModel>> getChapters(String _class,String _branch,String _subject) async {
    ...
    final response = await http.post(Uri.parse(Url),body:{"class": _class,"subject": _subject,"branch": _branch},);  
    if (response.statusCode == 200) {
      final data = json.decode(response.body) as List<dynamic>;
  ...
  static Future<void> updateName(String email,String name) async{
    ...
    var Response = await http.post(Uri.parse(apiUrl),body:{"email":email,"name":name},);
    if (Response.statusCode == 200) {
      Map<String, dynamic> userData = jsonDecode(Response.body);
      String message= userData['message'];
    } else {
